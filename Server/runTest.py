    ...
    for caseId in caseIds:
      ...
      if caseData:
        ...
        if caseData.param_type == 4:
          ...
          paramDatas = iatKeyValues.query.filter(db.and_(iatKeyValues.value_type == 2, iatKeyValues.pid == caseId)).all()
          if paramDatas:
...
def runScript(task_id):
  try:
    taskInfo = getTaskInfo(task_id) # 获取
